...
1. Set general input parameters:
   ...
   2. Specify the **Guider** (*B*) that the final image should simulate. If this is not known, check the APT file. If you plan to query APT for the guide star information, you will get a pop up if you selected guider does not match what is in the APT file.

2. If you are running MAGIC on the internal network to generate files for commissioning, for an **expected** observation number:
   ...
   Considering these parameters all together, the output files will be saved in the `.../{practice}/{car}/out/for_obs{obs}/` directory, with the root `for_obs{obs}_G{guider}`.
...
3. If you are running MAGIC on the internal network to generate files for commissioning, for an **unexpected** observation number:
   ...
   3. Specify the **Out** directory (*J*) to be `.../{practice}/{car}/`, filling in the directory for the current practice/rehearsal and the name of the current activity.
...
4. If you are running MAGIC outside of the internal network, or to generate test data:
...
6. Set image conversion parameters: (Note: The steps labelled “optional” below will create higher-fidelity simulations, but are not necessary when using MAGIC to generate FSW input or guiding override files.)
   ...
   4. (Optional) Add **Background Stars** to the final image.
    ...
    5. (Optional) Inspect the input image and determine if your PSF(s) meet one of the three scenarios that requires a change to how the code finds stars in the input image. If you are working with early commissioning and/or diffuse PSFs (but not in MIMF), check the **High Smoothing (Diffuse PSFs)** box (*G*). If you are working with MIMF PSFs, check the **Low Smoothing (MIMF)** box (*H*). If you are working with normal operations images, check the **Pixel-wise detection threshold (Normal Ops)** box (*I*).
        ...
        - For the low smoothing option, the flag applies very little smoothing to better find the bright knot on doughnut PSFs like in MIMF rather than finding the center of the PSF. This flag must be used for MIMF images in order for our flight software simulator files to be made correctly.
