...
MAGIC takes in an FGS image or a NIRCam image from *any* of their detectors that was taken with the CLEAR filter in. Note that the NIRCam weak lens0 (WL) filter will cause MAGIC to crash. MAGIC will convert this input image into a raw detector FGS image. In most cases, MAGIC can determine from the header information which instrument and detector the image is from, however if it can’t figure this out due to a missing or different header keyword, you will have to help it out by telling it which instrument and detector your input image comes from. The PSFs in this image can renormalized to the count rate of a specified Guide Star ID, a given count rate, or a given FGS magnitude. Additionally, background stars can be added to your seed image.
