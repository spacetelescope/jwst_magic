...
IF (ILRV(IFGS_CMD_REJ_CNT) != rejCount)
  SYSALERT INFO, "[TEST] @IFGS_CONFIG was rejected for Guider 1 (ACQ2)"
  PRINTF "\n@IFGS_CONFIG was rejected for Guider 1 (ACQ2)\n"
...
ELSE
  WAIT (ELRV(IFGS_CONFIG_STAT) != 'STARTED')
   ...
   IF (ELRV(IFGS_CONFIG_STAT) != 'SUCCESS')
       SYSALERT INFO, "[TEST] @IFGS_CONFIG did not complete successfully for Guider 1 (ACQ2)"
      PRINTF "\n@IFGS_CONFIG did not complete successfully for Guider 1 (ACQ2)\n"
      PRINTF "Press GO to continue, or ABORT ALL to abort the procedure\n"
      SUSPEND
   ENDIF
ENDIF
...
SYSALERT INFO, "[TEST] ACQing on Guider 1..."
PRINTF "\nACQing on Guider 1...\n"
...
SYSALERT INFO, "[TEST] ACQ G1 testing complete"
PRINTF "\nACQ G1 testing complete\n"
