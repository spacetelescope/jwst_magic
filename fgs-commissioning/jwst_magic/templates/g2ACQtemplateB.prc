...
IF (ILRV(IFGS_CMD_REJ_CNT) != rejCount)
   SYSALERT INFO, "[TEST] @IFGS_GUIDESTAR was rejected for Guider 2"
   PRINTF "\n@IFGS_GUIDESTAR was rejected for Guider 2"
...
ELSE
    ...
    IF (ELRV(IFGS_GUIDESTAR_STAT) != 'SUCCESS')
       SYSALERT INFO, "[TEST] @IFGS_GUIDESTAR did not complete successfully for Guider 2"
       PRINTF "\n@IFGS_GUIDESTAR did not complete successfully for Guider 2\n"
       PRINTF "Press GO to continue, or ABORT ALL to abort the procedure\n"
       SUSPEND
    ENDIF
ENDIF
	...
	;st gisim_FVTS_EXPOSE_SETUP('FVTS_FGSES_ACQ_G2')
...
;------------------------------------------------------------------------------
	...
	; **** THIS CONFIG WORKS ****    
	@IDAQ_CONFIGMSG OBSID="FVTS_ACQ1_G2",      \
	                EXPID=6,                   \
	                NINTS=3,                   \
			AVERAGE=1,                 \
	                NGROUPS=2,                 \
	...
	wait  (ELRV(IDAQ_EXP_STATUS1) == 'SETUP_COMPLETE')
        ...
        wait 3
        spaceWireAddr1 = ILRV(IDAQ_GUIDER2_ADDR)
        groupNum1 = ILRV(IDAQ_GUIDER2_GPI)
    ...
    @IDAQ_CONFIGMSG OBSID="FVTS_ACQ2_G2",   \
                    EXPID=6,                \
                    NINTS=5,                \
                    NGROUPS=2,              \
                    NFRAMES=1,              \
    ...
    wait  (ELRV(IDAQ_EXP_STATUS2) == 'SETUP_COMPLETE')
        ...
        wait 10
        spaceWireAddr2 = ILRV(IDAQ_GUIDER2_ADDR)
        groupNum2 = ILRV(IDAQ_GUIDER2_GPI)
