...
IF (ILRV(IFGS_CMD_REJ_CNT) != rejCount)
   SYSALERT INFO, "[TEST] @IFGS_CONFIG was rejected for Guider 2 (ACQ1)"
   PRINTF "\n@IFGS_CONFIG was rejected for Guider 2 (ACQ1)\n"
...
ELSE
    ...
    IF (ELRV(IFGS_CONFIG_STAT) != 'SUCCESS')
       SYSALERT INFO, "[TEST] @IFGS_CONFIG did not complete successfully for Guider 2 (ACQ1)"
       PRINTF "\n@IFGS_CONFIG did not complete successfully for Guider 2 (ACQ1)\n"
       PRINTF "Press GO to continue, or ABORT ALL to abort the procedure\n"
       SUSPEND
    ENDIF
ENDIF
