...
@pytest.mark.parametrize('crowded_field, guider, regfile_coords, ALLpsfs_coords', shift_to_id_attitude_parameters)
def test_shift_to_id_attitude(test_directory, crowded_field, guider, regfile_coords, ALLpsfs_coords):
    ...
    regfile = os.path.join(test_directory, 'shifted', file_root + '_regfile.txt')
    ALLpsfs = os.path.join(test_directory, 'shifted', file_root + '_ALLpsfs.txt')
    ...
    assert os.path.exists(ALLpsfs)

    # Make sure the shifted regfile is correct
    regfile_cat = asc.read(regfile)
    coords = [(x, y) for (x, y) in regfile_cat['x', 'y']]
    assert coords == regfile_coords

    # Make sure the shifted ALLpsfs is correct
    ALLpsfs_cat = asc.read(ALLpsfs)
    coords = [(x, y) for (x, y) in ALLpsfs_cat['x', 'y']]
    assert coords == ALLpsfs_coords

    # Make sure the location of the PSFs in the image matches the ALLpsfs
