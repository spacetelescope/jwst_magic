...
# Setup
from math import radians, cos, sin, degrees, acos
import numpy as np
from astropy.io import ascii
import glob
import matplotlib.pyplot as pl
import xml.etree.cElementTree as etree
from pysiaf.utils import rotations, polynomial
...
A = rotations.attitude(V2Aim, V3Aim, a0, d0, pa)
#print('Attitude Matrix')
#print(A)
(a1, d1) = rotations.pointing(A, V2Aim, V3Aim)
...
for s in range(nseg):
    ...
    (a, d) = rotations.pointing(A, V2, V3)
    apoint[s] = a
    dpoint[s] = d
...
for p in range(nseg):
    ...
    xDet[p] = XSciRef + polynomial.poly(C, xIdlSeg[p], yIdlSeg[p], order)
    yDet[p] = YSciRef + polynomial.poly(D, xIdlSeg[p], yIdlSeg[p], order)
...
def sep(a1, d1, a2, d2):
    cossep = cos(radians(a1 - a2)) * cos(radians(d1)) * cos(radians(d2)) + \
             sin(radians(d1)) * sin(radians(d2))
