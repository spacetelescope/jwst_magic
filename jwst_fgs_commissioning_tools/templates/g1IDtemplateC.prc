...
IF (ILRV(IFGS_CMD_REJ_CNT) != rejCount)
   SYSALERT INFO, "[TEST] @IFGS_REFCOUNT was rejected for Guider 1"
   PRINTF "\n@IFGS_REFCOUNT was rejected for Guider 1"
...
ELSE
    ...
    IF (ELRV(IFGS_REFCNT_STAT) != 'SUCCESS')
       SYSALERT INFO, "[TEST] @IFGS_REFCOUNT did not complete successfully for Guider 1"
       PRINTF "\n@IFGS_REFCOUNT did not complete successfully for Guider 1\n"
       PRINTF "Press GO to continue, or ABORT ALL to abort the procedure\n"
       SUSPEND
    ENDIF
ENDIF
