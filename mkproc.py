...
class mkproc(object):
    ...
    def __init__(self, guider, root, x, y, c, step, thresh_factor=0.5, out_dir=None,
        ...
        if step == 'ID':
            self.create_ID_proc_file(root, guider, x, y, c,
    ...
    def create_ID_proc_file(self, root, guider, x, y, c, thresh_factor):
        eol = '\n'
        nref = len(x)-1
    ...
    def create_ACQ_proc_file(self, root, guider):
        ...
        i1, x1, y1, c1 = np.loadtxt(os.path.join(self.out_dir, 'stsci',
                                                 '{}_G{}_ACQ1.stc'.format(root,
                                                                          guider))).T
        threshgs = 0.50 * c1

        i2, x2, y2, c2 = np.loadtxt(os.path.join(self.out_dir, 'stsci',
        ...
        with open(os.path.join(self.out_dir, 'dhas',
            ...
            file_out.write('@IFGS_GUIDESTAR {0}, 2, {1:12.4f}, {2:12.4f}, \
                           {3:12.4f}, {4:8d}'.format(self.guider, float(x1),
                                                     float(y1), float(c1),
                                                     int(threshgs)))
            self.write_from_template(self.templateB, file_out)
